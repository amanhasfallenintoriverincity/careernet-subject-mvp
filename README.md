...
- Google Gemini Interactions API 문서 기준 REST `interactions` 형식으로 작동: `x-goog-api-key` 인증 헤더, `input`, `generation_config`, `response_mime_type`, `previous_interaction_id`, `outputs[].text`/`steps[].content[].text` 응답 파싱
...
- `lib/gemini-api.ts`: Google Gemini Interactions API 요청 생성, 이전 상호작용 연결, 응답 텍스트/interaction ID 추출
